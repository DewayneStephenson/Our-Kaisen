...
// biome-ignore lint/complexity/noStaticOnlyClass: Shared namespace for embed factories.
export class EmbedCreator {
    ...
    static lobby(lobby: Lobby) {
        const players = lobby.players.map(
            (id) => lobby.botNames.get(id) ?? `<@${id}>`,
        );
        const timerText = [
        ...
        return new EmbedBuilder()
            ...
            .addFields(
                ...
                },
                {
                    ...
                    value:
                        lobby.title ??
                        "Automatically generated when the game starts.",
                    inline: false,
                },
                {
    ...
    static game(game: Game) {
        const missions = game.missionResults
            .map(
                (result, index) =>
                    `${index + 1}. ${EmbedCreator.formatMissionStatus(result)}`,
            )
            .join("\n");
        ...
        return new EmbedBuilder()
            ...
            .addFields(
                ...
                },
                {
                    name: "Roles",
                    value:
                        game.lobby.roles.length > 0
                            ? game.lobby.roles
                                  .map((role) => RoleManager.ROLES[role].name)
                                  .join(", ")
                            : "No roles selected.",
                },
                {
    ...
    static mission(game: Game) {
        ...
        const missionNumber =
            missionIndex === -1 ? game.missionCount : missionIndex + 1;
        const teamSizes = getMissionTeamSizes(game.players.length);
        const requiredTeamSize =
            teamSizes[missionIndex] ?? teamSizes[teamSizes.length - 1] ?? 0;
        ...
        const approvalVotes = Object.keys(game.expeditionVotes).length
            ? Object.entries(game.expeditionVotes)
                  .map(
                      ([voterId, vote]) =>
                          `${EmbedCreator.playerLabel(game, voterId)}: ${vote === "approve" ? "Approve" : "Reject"}`,
                  )
                  .join("\n")
            ...
            .addFields(
                {
                    name: "Mission Leader",
                    value: leader
                        ? EmbedCreator.playerLabel(game, leader.discordId)
                        : "No leader.",
                    inline: true,
                },
                {
    ...
    static missionResult(
        ...
        return new EmbedBuilder()
            .setTitle(
                `Mission ${missionNumber} ${success ? "Succeeded" : "Failed"}`,
            )
    ...
    static missions(game: Game) {
        ...
        return new EmbedBuilder()
            ...
            .setFooter({
                text: `Current phase: ${EmbedCreator.formatPhase(game.phase)}`,
            });
    }

    static leaders(game: Game) {
        const nextLeaders = Array.from(
            { length: Math.min(5, game.players.length) },
            (_, index) => {
    ...
    static roundResult(game: Game) {
        const winners =
            game.players
                .filter(
                    (player) => player.role.alignment === game.winnerAlignment,
                )
                .map(
                    (player) =>
                        `${EmbedCreator.playerLabel(game, player.discordId)} — ${player.role.roleName}`,
                )
                .join("\n") || "None";
        const losers =
            game.players
                ...
                .join("\n") || "None";
        const assassin = game.players.find(
            (player) => player.discordId === game.sealingAssassinId,
        );
        const target = game.players.find(
            (player) => player.discordId === game.sealingTargetId,
        );
        const sealingText =
            assassin && target
    ...
    static player(game: Game, userID: string) {
        return EmbedCreator.playerDM(game, userID);
    }

    static playerDM(game: Game, userID: string) {
        const player = game.players.find(
            (currentPlayer: Player) => currentPlayer.discordId === userID,
        );
        ...
        const teamLabel =
            role.roleName === RoleManager.ROLES.gojo.name
                ? "Enemies"
                : "Teammates";
        const emptyLabel =
            role.roleName === RoleManager.ROLES.gojo.name
        ...
        return new EmbedBuilder()
            ...
            .addFields(
                ...
                {
                    name: teamLabel,
                    value: teammates.length ? teammates.join("\n") : emptyLabel,
                    inline: false,
                },
            )
