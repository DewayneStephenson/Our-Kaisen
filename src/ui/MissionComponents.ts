...
function interactionPlayerName(game: Game, playerId: string, fallback: string) {
    return (
        game.lobby.botNames.get(playerId) ??
...
export function buildMissionComponents(game: Game) {
    ...
    const powerRows = game.players
        .filter(
            (player) =>
                player.role?.power?.target && player.role.power.uses > 0,
        )
        .slice(0, 4)
        .map((player) =>
            new ActionRowBuilder<StringSelectMenuBuilder>().addComponents(
                new StringSelectMenuBuilder()
                    ...
                    .setPlaceholder(
                        `${player.role.power?.PowerName ?? "Power"}: choose a target`,
                    )
                    .setMinValues(1)
                    .setMaxValues(1)
                    .addOptions(
                        game.players
                            .filter(
                                (target) =>
                                    target.discordId !== player.discordId,
                            )
                            .map((target) => ({
                                label: interactionPlayerName(
                                    game,
                                    target.discordId,
                                    target.username,
                                ),
