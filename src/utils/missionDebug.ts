...
export async function publishRoundResult(game: Game) {
    if (game.roundResultAnnounced || !game.winnerAlignment || !game.lobby.message?.channel.isSendable()) {
        return;
    }

    await game.lobby.message.channel.send({ embeds: [EmbedCreator.roundResult(game)] });
    const missionHistory = game.missionResults
        .map((result, index) => {
            const votes = game.missionVoteResults[index]?.map(vote => vote === "pass" ? "succeed" : "fail").join(", ") ?? "none";
...
export async function publishPendingMissionReveals(game: Game) {
    if (!game.pendingMissionReveals.length || !game.lobby.message?.channel.isSendable()) {
        return;
    }
