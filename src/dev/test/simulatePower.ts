...
export default {
    ...
    async execute(interaction: ChatInputCommandInteraction, client: Client) {
        ...
        if (game.phase !== "MISSION") {
            return interaction.reply({
                content: "Powers can only be simulated during a mission.",
                flags: MessageFlags.Ephemeral,
            });
        }
        ...
        const actor = game.players.find(
            (player: Player) =>
                ...
                player.role?.power?.target &&
                player.role.power.uses > 0,
        );
        const power = actor?.role.power;

        if (!actor || !power?.target) {
            return interaction.reply({
                content: "No bot has a targeted power with uses remaining.",
                flags: MessageFlags.Ephemeral,
            });
        }

        const candidates = game.players.filter(
            (player: Player) => player.discordId !== actor.discordId,
        );
        ...
        if (
            !target ||
            !candidates.some(
                (player: Player) => player.discordId === target.discordId,
            )
        ) {
            return interaction.reply({
                content: "Choose another bot in this game.",
                flags: MessageFlags.Ephemeral,
            });
        }
        ...
        if (power.revealTiming === "after_mission") {
            ...
            return interaction.reply({
                content: `${power.PowerName} was used; its result will be revealed after the mission.`,
                flags: MessageFlags.Ephemeral,
            });
        }
